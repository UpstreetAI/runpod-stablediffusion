...
RUN curl -s -S -L https://raw.githubusercontent.com/pyenv/pyenv-installer/master/bin/pyenv-installer | bash && \
	...
	pyenv install-latest "3.10.9" && \
	pyenv global $(pyenv install-latest --print "3.10.9") && \
	pip install "wheel<1"
COPY .cog/wheel/cog-0.0.1.dev-py3-none-any.whl /tmp/cog-0.0.1.dev-py3-none-any.whl
RUN --mount=type=cache,target=/root/.cache/pip pip install /tmp/cog-0.0.1.dev-py3-none-any.whl
COPY .cog/wheel/requirements.txt /tmp/requirements.txt
RUN --mount=type=cache,target=/root/.cache/pip pip install -r /tmp/requirements.txt
WORKDIR /src
COPY . /src
RUN sh builder/setup.sh
EXPOSE 5000
